...
I place the usage of tools as the first important thing to talk about. The reason is that a good choice of coding environment will to a large extent decide the quality of the code.
#### IDEs

Many people have different preferences for IDEs. I would like to recommend **VS Code** as an extension. The merit of VS Code is that you can add any extensions and build up your own workstation. Choose a good appearance and the placement of toolbox bars.
...
- [Grammar Checker](https://code.visualstudio.com/docs/python/linting). Usually, in the IDE, it will highlight the warnings and potential errors before compiling or running the program. But Linting is also another important part during the writing of your code. It will give hints to better improve the code, avoid potential bugs, mistakes causing overheads, etc. It is highly recommended to have the Grammar checker prepared.
#### AI Assistant
...
### Project Management
Project management is usually very important, especially if there is a project with a large team. The Github and own GitLan are both good options. Git provides the flexibility of pull, push, commit tracing, pull requests, rolling back, and so on for the easy management of a project. 
...
- Provide a proper description of your commit. The reason for doing this is that others can quickly understand your change without reading your code and it will make it more convenient if you need to roll back.

 1. Capitalize the first letter of the message.
 2. Always started with a verb.
 3. The starting verb should be in past tense, such as "Added..." and "Supported...".
...
- Even if it is a personal project, using Git does not have any drawbacks. The reason is that I can track progress and roll back when necessary. If I want to involve other people, it will also be easier.
...
### Code Logistics
Code logistics is another important component of having good code. However, this may vary from case to case.
...
- **Trojan Horse**: Always be careful when you copy and paste others' code. Even if their code does not have bugs or vulnerabilities, you may misuse them when you do the copy and paste. Some copying and pasting is fine, for example,
 ```
 loss.backward()
 optimizer.step()
 optimizer.zero_grad()
...
- **Misuse of concepts**: In different programming languages, there are usually helpful features. For example, inheritance, pointers, references, and so on. But one thing we need to do is make sure that they are properly used. For example, sometimes, a callback function is a better solution than inheritance or a direct function call. 
